...
function buildRateLimitHandler(message) {
    return (req, res) => {
        ...
        const view = req.originalUrl.includes("/login")
            ? "login"
        ...
        return res.status(429).render(view, {
            ...
            values: {
                ...
                customShortId:
                    typeof req.body?.customShortId === "string" ? req.body.customShortId : "",
...
export const urlCreateRateLimiter = rateLimit({
    ...
    handler: buildRateLimitHandler(
        "Too many URL creation requests. Please slow down and try again shortly."
    )
});
